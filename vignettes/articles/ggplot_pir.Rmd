...
poa1980 <- csapAIH::ler_popbr(ano = 1980) %>% 
  filter(munic_res == "431490",
         fxetaria != "I000") %>% 
  mutate(fxetar5 = fxetar.det_pra_fxetar5(fxetaria),
         sexo = factor(sexo, labels = c("masc", "fem"))) %>%
  select(-c(munic_res, situacao, fxetaria))  

poa2010 <- csapAIH::ler_popbr(ano = 2010) %>% 
  filter(munic_res == "431490") %>% 
  mutate(fxetar5 = fxetar.det_pra_fxetar5(fxetaria),
         sexo = factor(sexo, labels = c("masc", "fem"))) %>% 
  select(-c(munic_res, situacao, fxetaria)) 

poa2020 <- csapAIH::popbr2000_2021(2020, munic = "431490") %>% 
  rename(populacao = pop) %>% 
  select(-c(CO_UF, UF_SIGLA, REGIAO, mun)) 
...
gr2010 <- ggplot_pir(poa2010, "fxetar5", "sexo", "populacao", nsize = nsize) +
  labs(x = "", fill = "") +
  ggtitle("Porto Alegre, RS, 2010") 
gr2020 <- ggplot_pir(poa2020, "fxetar5", "sexo", "populacao", nsize = nsize) +
  labs(x = "", fill = "") +
...
```{r echo=FALSE}
data("POPBR2012")
# plot_pir(POPBR12[POPBR12$MUNIC_RES == "430520",])
```
