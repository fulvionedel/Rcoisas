...
Mas os valores da tabela estão em formato caractere e não numérico, o que impede a execução de operações matemáticas. Por isso foi rebatizada de `tabuleiro2` e seu desenvolvimento descontinuado. Ainda está no pacote porque ainda a tenho em várias aulas `r emo::ji("sleepy")`. É desaconselhável seu uso em novos scripts.
...
O gráfico pode ser rapidamente produzido desenhando o resultado de `prop.table` e `table` sobre `fxetarNM(x)`.
...
> [...] cardiovascular é `formatL((tabolero$RP-1)*100, digits = 0)`% maior no sexo feminino que no masculino (RP = `formatL(tabolero$RP, 2)`; IC~95~ `formatL(tabolero$lci.rp, 2)` a `formatL(tabolero$uci.rp, 2)`).
